...
{
    private static readonly string Path = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + @"\OpenShock\Sentry\config.json";
    
    private readonly ILogger<ConfigManager> _logger;
    public SentryConfig Config { get; }
    ...
    public ConfigManager(ILogger<ConfigManager> logger)
    {
        ...
        if (File.Exists(Path))
        {
            ...
            if (!string.IsNullOrWhiteSpace(json))
            {
                ...
                try
                {
                    config = JsonSerializer.Deserialize<SentryConfig>(json, Options);
                }
                catch (Exception e)
                {
